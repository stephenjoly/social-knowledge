...
| Settings and onboarding | API keys and connections are valuable | Replace the long settings page with a left navigation by topic; consider an in-app, downloadable Apple Shortcut setup flow. |
...
- **One UI roadmap item, several releases:** the visual refresh is one program with independently reviewable filter/search, detail, Ask, Activity, Settings, and responsive milestones. Content quality and failure handling are earlier because they affect trust and daily use.
...
- **Open product choices:** confirm TikTok scope; choose the final Inbox/Library names; define category versus tag ownership and edit behavior; choose whether user notes change exported Markdown; decide the supported Obsidian sync model. These choices do not block the first UI and insight-quality work.
